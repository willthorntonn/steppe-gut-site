...
// A paged row of portrait cards - the reference gut-health page's recipe
// carousel, generalised so the diet page can use it for both the recipe rail
// and the "more from Gut Health" row. Same mechanics as
// components/gut-health/WaysToTakeIt and components/home/ReadsCarousel: a
// scroll-snap track so the row is natively swipeable and keyboard-scrollable,
// with arrows and dots driven off the measured scroll position rather than a
// transform track, because the number of cards on screen changes at every
// breakpoint. No card kicker label: the reference sets a small grey category
// over each image and it is deliberately dropped here.
//
// Props: heading, intro (optional), items ({ id, title, note, to, image }),
// trackId (unique per instance on a page, wires the arrows to their track).
...
export default function CardRail({ heading, intro, items, trackId, ratio = "3 / 4" }) {
  ...
  return (
    <div>
      ...
      {/* The track clips at the content measure, not the viewport, so the row
          sits on the same column as every other section while the next card is
          still cut mid-frame - which is what signals there is more. */}
